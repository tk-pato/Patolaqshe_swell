...
$img_dir = get_stylesheet_directory();
$img_uri = get_stylesheet_directory_uri();

// 背景画像（PC/SP）: 無ければプレースホルダーへフォールバック
$bg_pc_rel = '/img/bust-issues-bg-pc.jpg';
$bg_sp_rel = '/img/bust-issues-bg-sp.jpg';
$ph_uri    = $img_uri . '/img/ourprices-bg-placeholder-1920x1080.svg';

$bg_pc = file_exists($img_dir . $bg_pc_rel) ? ($img_uri . $bg_pc_rel) : $ph_uri;
$bg_sp = file_exists($img_dir . $bg_sp_rel) ? ($img_uri . $bg_sp_rel) : $bg_pc;

// チェックアイコン: 子テーマ img/check.png があれば使用、無ければSVGで代替
$check_rel = '/img/check.png';
$check_uri = file_exists($img_dir . $check_rel) ? ($img_uri . $check_rel) : '';
...
<section id="bust-issues" class="ptl-section ptl-bustIssues">
    <div class="ptl-bustIssues__bg" aria-hidden="true">
        <picture>
            <source media="(max-width: 767px)" srcset="<?php echo esc_url($bg_sp); ?>">
            <img src="<?php echo esc_url($bg_pc); ?>" alt="" loading="lazy" decoding="async">
        </picture>
        <div class="ptl-bustIssues__overlay"></div>
    </div>
