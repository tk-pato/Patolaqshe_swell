...
add_action('wp_enqueue_scripts', function () {
  ...
  // CSS PC版
  $privacy_pc = get_stylesheet_directory() . '/css/pc/privacy-modal-pc.css';
  if (file_exists($privacy_pc)) {
    wp_enqueue_style(
      'ptl-privacy-modal-pc',
      get_stylesheet_directory_uri() . '/css/pc/privacy-modal-pc.css',
      array(),
      filemtime($privacy_pc),
      'screen and (min-width: 768px)'
    );
  }

  // CSS SP版
  $privacy_sp = get_stylesheet_directory() . '/css/sp/privacy-modal-sp.css';
  if (file_exists($privacy_sp)) {
    wp_enqueue_style(
      'ptl-privacy-modal-sp',
      get_stylesheet_directory_uri() . '/css/sp/privacy-modal-sp.css',
      array(),
      filemtime($privacy_sp),
      'screen and (max-width: 767px)'
    );
  }

  // JavaScript
  $privacy_js = get_stylesheet_directory() . '/js/privacy-modal.js';
  if (file_exists($privacy_js)) {
    wp_enqueue_script(
      'ptl-privacy-modal',
      get_stylesheet_directory_uri() . '/js/privacy-modal.js',
      array(),
      filemtime($privacy_js),
      true
    );
  }
}, 30);
