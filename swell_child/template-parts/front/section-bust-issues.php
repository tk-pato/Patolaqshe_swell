...
<section id="bust-issues" class="ptlIssues ptlNavHero is-translucent<?php echo $has_bg ? ' has-bg' : ''; ?>" data-parallax="bg" data-parallax-target=".ptlNavHero__bg" data-parallax-speed="<?php echo esc_attr($p_speed); ?>" data-parallax-clamp="0.18" data-parallax-distance="240" data-parallax-scale="1.55">
    <?php if ($has_bg): ?>
        <div class="ptlIssues__bg ptlNavHero__bg" aria-hidden="true">
            <?php if ($video_url): ?>
                <video class="ptlIssues__video ptlNavHero__video" src="<?php echo esc_url($video_url); ?>" autoplay muted loop playsinline></video>
            <?php else: ?>
    ...
    <div class="ptl-section__inner">
        <div class="ptlIssues__titleArea">
            <h2 class="ptl-section__title is-onImage">パトラクシェの<br>ハイブリッドバストアップは<br>このような方におすすめです</h2>
            <p class="ptl-section__subtitle">BUST-ISSUES</p>
        </div>

        <!-- 新：8つの悩みチェックリスト -->
        <div class="ptlIssues__card">
            <ul class="ptlIssues__list" role="list">
