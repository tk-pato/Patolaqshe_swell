...
<section id="page-navigation" class="ptlNavHero is-translucent<?php echo $has_bg ? ' has-bg' : ''; ?>" data-parallax="bg" data-parallax-target=".ptlNavHero__bg" data-parallax-speed="<?php echo esc_attr($p_speed); ?>" data-parallax-clamp="0.18" data-parallax-distance="240" data-parallax-scale="1.55">
    <?php if ($has_bg): ?>
        <div class="ptlNavHero__bg" aria-hidden="true">
            <?php if ($video_url): ?>
                <video class="ptlNavHero__video" src="<?php echo esc_url($video_url); ?>" autoplay muted loop playsinline></video>
            <?php else: ?>
                <picture class="ptlNavHero__image">
                    <?php if (! empty($bg_sp)): ?>
    ...
    <div class="ptl-section__inner">
        <h2 class="ptl-section__title is-onImage">サイトナビ</h2>
        ...
        <div id="ptl-nav-menu" class="ptlNavHero__grid ptl-nav-collapsible">
            <?php
            ...
            foreach ($items as $it): if (empty($it['label'])) continue;
                ...
                if (!$icon_html && $slug) {
                    ...
                    if (file_exists($png_wh)) {
                        $icon_html = '<img src="' . esc_url($icon_dir_uri . $slug . '_wh.png') . '" alt="' . esc_attr($label) . '" width="256" height="256" loading="lazy" decoding="async">';
                    } else {
                        $png = $icon_dir_abs . $slug . '.png';
                        if (file_exists($png)) {
                            $icon_html = '<img src="' . esc_url($icon_dir_uri . $slug . '.png') . '" alt="' . esc_attr($label) . '" width="256" height="256" loading="lazy" decoding="async">';
                        }
                    }
                }
                if (!$icon_html) {
            ...
            ?>
                <a class="ptlNavHero__btn<?php echo !empty($it['modal_class']) ? ' ' . esc_attr($it['modal_class']) : ''; ?>" href="<?php echo esc_url($href); ?>"<?php echo !empty($it['modal_id']) ? ' data-modal-id="' . esc_attr($it['modal_id']) . '"' : ''; ?>>
